...
module Tests = 
    ...
    [<Tests>]
    let t4 = makeExpectoTestList id id (makeLiteral) "Valid Literal Tests" [
                ("-245", Ok({K=146u ; R=1; I=true}))
                ("123", Ok({K=146u ; R=1; I=true}))
                ("0xFFFFFFF0", Ok({K=146u ; R=1; I=true}))
    ...
    [<Tests>]
    let t5 = makeExpectoTestList makeLiteral id (Result.map(litValue)) "Valid LiteralVal (Inversion) Tests" [
                ("-44", Ok(4294967252u))
                ("44", Ok(44u))
    ...
    [<Tests>]
    let t6 = makeExpectoTestList tokenize id (makeOperands) "Tokens Validate Tests" [
